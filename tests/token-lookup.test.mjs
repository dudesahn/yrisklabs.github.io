...
import {
  intakeNetworks, intakeNetwork, parseTokenInput, checksumAddress, decodeTokenText,
...
test("EIP-55 reference addresses normalize from every casing and explorer links", () => {
  // Official test vectors: https://eips.ethereum.org/EIPS/eip-55
  for (const expected of [
    ...
    for (const input of [expected, expected.toLowerCase(), expected.toUpperCase()]) {
      assert.equal(checksumAddress(input), expected);
      assert.equal(parseTokenInput(input, "Ethereum").address, expected);
      assert.equal(parseTokenInput(`https://arbiscan.io/token/${input}`, "Ethereum").address, expected);
    }
  }
  for (const manual of ["", "N/A", "0x123", "0x" + "g".repeat(40)]) assert.equal(checksumAddress(manual), manual);
});
